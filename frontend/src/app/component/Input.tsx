...
interface IInput {
    type: 'text' | 'password' | 'multi' | 'date' | 'date-range';
    form: Form;
...
export default function Input(props: IInput) {
    ...
    const selectChangeHandler =
        ...
        (action) => {
            props.setForm((form) =>
                form.setValue(name, FormMultiTypeUtilities.findByKey(props.values, action.target.value)),
            );
        };
    ...
    const dateChangeHandler =
        ...
        (action: Dayjs) => {
            if (action && action.isValid())
                props.setForm((form) =>
                    form.setValue(name, action.hour(0).minute(0).second(0).utc(true).toISOString()),
                );
    ...
    switch (props.type) {
                ...
                <FormControl fullWidth error={props.form.haveError(props.name)}>
                    ...
                    >
                        {props.values?.map((value, index) => {
                            return (
                                <MenuItem key={index} value={value.getKey()}>
                                    {value.print()}
                                </MenuItem>
                            );
                        })}
        ...
        case 'date-range':
            element = <Typography>To implement</Typography>;
    }
