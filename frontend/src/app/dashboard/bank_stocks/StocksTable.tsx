...
export function StocksTable(props: IProps) {
    ...
    function refreshTable() {
        ...
        api.Stock.List({ sort: sort.toUrlString() })
            .then((stocks) => {
    ...
    const clickActiveStockHandler =
        (id: number): MouseEventHandler<SVGSVGElement> =>
        () => {
                ...
                <Table stickyHeader size={isMobile ? 'small' : 'medium'}>
                    <TableHead>
                        <TableRow>
                            ...
                            <TableCell style={{ width: '50px' }}>
                                <TableSortLabel>Rendimento</TableSortLabel>
                            </TableCell>
                    ...
                    <TableBody>
                        ...
                        {(!stocks || stocks.length === 0) && (
                            <TableRow>
                                <TableCell colSpan={4}>
                                    <Typography fontStyle="italic" color="textSecondary" align="center">
                                        Inserire la prima azione
                                    </Typography>
                                </TableCell>
                            </TableRow>
                        )}
                        {stocks?.map((value, index) => (
                            <TableRow
                                key={index}
                                sx={{ backgroundColor: value.id === props.clickedStock ? '#c1121f30' : undefined }}
                            >
                                ...
                                <TableCell align="right">
                                    {convertNumberToPercentage(value.currentValue / value.resourcesInvested - 1)}
                                </TableCell>
                                <TableCell sx={{ display: 'flex', gap: 1.5, justifyContent: 'right' }}>
                                ...
                                <TableCell>
                                    <Box sx={{ display: 'flex', gap: 2 }}>
                                        <FontAwesomeIcon
                                            ...
                                            icon={faPen}
                                            onClick={props.editStockClick(value.id)}
                                        />
                                        <FontAwesomeIcon
                                            style={{ cursor: 'pointer' }}
                                            icon={faTrash}
                                            onClick={props.deleteStockClick(value.id)}
                                        />
                                        <FontAwesomeIcon
                                            style={{ cursor: 'pointer' }}
                                            icon={faArrowRight}
                                            onClick={clickActiveStockHandler(value.id)}
                                        />
                                    </Box>
                                </TableCell>
                            </TableRow>
