...
export default function Page() {
    ...
    const hideDialog = (refreshTable: boolean) => {
        if (refreshTable && stockTableRef.current != null) stockTableRef.current.refreshTable();
    ...
    return (
        <Box display="flex" flexDirection="column" gap={1} height="100%">
            <DeleteMovementDialog
                open={movementDeleteId !== undefined}
                onClose={hideMovementDialog}
                movement={movementDeleteId}
            />
            ...
            <StockMovementDialog
                ...
                onClose={hideMovementDialog}
                stockId={activeStock}
                stockMovementId={stockMovementEditId}
            />
            <Box display="flex" flexDirection="row" justifyContent="space-between">
                <Box display="flex" flexDirection="row">
                    <Button
                        ...
                        onClick={clickAddNewStockHandler}
                    >
                ...
                <Box display="flex" flexDirection="row">
                    <Button
                        variant="outlined"
                        startIcon={<FontAwesomeIcon icon={faPlus} />}
                        disabled={activeStock === undefined}
                        onClick={clickAddNewStockMovementHandler}
                    >
            ...
            <Box display="grid" gridTemplateColumns="1fr 550px" height="100%" gap={1}>
                <StocksTable
                    ref={stockTableRef}
                    loading={false}
                    clickedStock={activeStock}
                    setClickedStock={openMovementTableHandler}
                    editStockClick={clickOnEditStockHandler}
                    deleteStockClick={clickOnDeleteStockHandler}
                />
                {activeStock === undefined ? (
                    <Paper sx={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Typography fontSize="2em" fontStyle="italic">
                            Seleziona un&apos;azione
                        </Typography>
                    </Paper>
                ) : (
                    <StockMovementTable
                        ref={stockMovementTableRef}
                        clickedStock={activeStock}
                        editStockClick={clickOnEditStockMovementHandler}
                        deleteStockClick={clickOnDeleteStockMovementHandler}
                    />
                )}
            </Box>
