...
export default function StockMovementDialog(props: IProps) {
    ...
    useEffect(() => {
        if (props.stockId === undefined || props.stockId === null) return;
    ...
    useEffect(() => {
        if (props.stockMovementId === undefined || props.stockMovementId === null) return;
        ...
        api.StockMovement.Get(props.stockMovementId)
            .then((stockMovement) => {
                ...
                setForm((form) =>
                    form
                        .setValue('description', stockMovement.description || '')
                        .setValue('date', stockMovement.date || '')
                        .setValue('value', stockMovement.value.toString())
                        .setValue(
                            'wallet',
                            stockMovement.bank_deposit?.wallet &&
                                new WalletPrintable(stockMovement.bank_deposit.wallet),
                        ),
                );
    ...
    function loadWallet(): Promise<void> {
        return api.Wallet.List({ sort: '!favorite-name' }).then((wallets) => setWallets(wallets));
    }

    const saveHandler = () => {
        setForm((form) => form.check());
        if (form.isCheckFail()) return;
        ...
        let stockMovement: CreateStockMovement = {
            ...
            wallet:
                movementType === 'deposit'
                    ? form.getStringValue('wallet')
                        ? parseInt(form.getStringValue('wallet'))
                        : null
                    : null,
            stock: {
                id: props.stockId,
            ...
            api.StockMovement.Create(stockMovement)
                ...
                .catch((err: ResponseError) => {
                    switch (err.code) {
                        ...
                        case 302:
                            enqueueSnackbar(
                                "Non puoi inserire un movimento con data antecedente all'ultimo movimento",
                                { variant: 'error' },
                            );
                            break;
                    }
        ...
        <Dialog open={props.open} onClose={props.onClose} TransitionComponent={TransitionDialog}>
            ...
            <DialogActions>
                ...
                <Button onClick={saveHandler} disabled={loading}>
                    {props.stockMovementId == null ? 'Salva' : 'Modifica'}
                </Button>
            </DialogActions>
        </Dialog>
    );
