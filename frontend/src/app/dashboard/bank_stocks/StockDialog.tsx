...
export default function StockDialog(props: IProps) {
    ...
    const saveHandler = () => {
        ...
        let stock: Stock = {
            ...
            subscriptionDate:
                form.getStringValue('date') ?? dayjs.utc().hour(0).minute(0).second(0).millisecond(0).toISOString(),
    ...
    return (
        <Dialog open={props.open} onClose={props.onClose} TransitionComponent={TransitionDialog}>
            ...
            <DialogContent>
                <DialogContentText>Inserire i dati della nuova azione bancaria</DialogContentText>
                <Grid container spacing={2} sx={{ marginTop: 1 }} component="form">
                    <Grid size={{ xs: 12 }}>
                        <Input type="text" form={form} setForm={setForm} name="name" label="Nome" disabled={loading} />
                    </Grid>
            ...
            <DialogActions>
                ...
                <Button onClick={saveHandler} disabled={loading}>
                    {props.stockId == null ? 'Salva' : 'Modifica'}
                </Button>
            </DialogActions>
        </Dialog>
    );
